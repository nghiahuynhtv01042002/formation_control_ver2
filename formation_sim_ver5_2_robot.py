...
pid_vx = PID(15, 1.5, 0.1)
pid_vy = PID(15, 1.5, 0.1)
pid_w  = PID(15,1.5,0.1)

leader_trajectory = {'x': [], 'y': [], 'theta': []}
follower_trajectory = {'x': [], 'y': [], 'theta': []}
...
def update(frame):
    global leader, follower, leader_circle, follower_circle, leader_arrow, follower_arrow, time_step
    time_step+=1
    print(time_step)
    ...
    # print("vr_leader: {}, vl_leader: {}".format(vr_leader,vl_leader))
    ...
    # w = (-vx*np.cos(np.pi*0.5 - follower.theta) + vy*np.sin(np.pi*0.5 - follower.theta))/0.0325
    w = (-vx*np.sin(follower.theta) + vy*np.cos(follower.theta))/0.0325
    ...
    if np.sqrt((leader.x - follower.x)**2+(leader.y - follower.y)**2)<=L - 0.005:
        v_r = v_l = 0
    ...
    follower_trajectory['x'].append(follower.x)
    follower_trajectory['y'].append(follower.y)
    follower_trajectory['theta'].append(np.rad2deg(follower.theta))
...
axs[0].plot(follower_trajectory['x'], label='Follower')
...
axs[1].plot(follower_trajectory['y'], label='Follower')
# axs[1].plot(follower2_trajectory['y'], label='Follower 2')
axs[1].set_ylabel('y')
axs[1].legend()

axs[2].plot(leader_trajectory['theta'], label='Leader')
axs[2].plot(follower_trajectory['theta'], label='Follower')
